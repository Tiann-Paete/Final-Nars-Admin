...
const db = mysql.createPool({
  ...
  connectionLimit: 10,
  queueLimit: 0,
  timezone: '+08:00'  // Use offset instead of timezone name
  ...
  try {
    switch (method) {
      ...
      case 'POST':
        ...
        case 'PUT':
          if (pathname.startsWith('/api/orders/') && pathname.endsWith('/update-stocks') && method === 'PUT') {
            const orderId = pathname.split('/')[3]; // This will get the order ID from the URL
            return handleUpdateStocks(req, res, orderId);
...
async function handleGetProductDetails(req, res) {
  ...
  try {
    const [totalSalesResult] = await db.query(`
      ...
      WHERE 
        ${dateCondition}
        AND (o.status = 'Delivered' OR o.status = 'Received')
  ...
  try {
    const [results] = await db.query(`
      SELECT 
        ...
        SUM(CASE WHEN (status = 'Delivered' OR status = 'Received') AND in_sales_report = 1 THEN 1 ELSE 0 END) as orders,
        CAST(COALESCE(SUM(
          CASE WHEN (status = 'Delivered' OR status = 'Received') AND in_sales_report = 1 
          THEN subtotal
          ELSE 0 
          END
  ...
  try {
    const [results] = await db.query(`
      SELECT 
        ...
        SUM(CASE WHEN (status = 'Delivered' OR status = 'Received') AND in_sales_report = 1 THEN 1 ELSE 0 END) as orderCount,
        CAST(COALESCE(SUM(
          CASE WHEN (status = 'Delivered' OR status = 'Received') AND in_sales_report = 1 
          THEN subtotal  
          ELSE 0 
...
async function handleGetSalesData(req, res) {
  ...
  try {
    const [salesResult] = await db.query(`
      ...
      FROM orders
      WHERE ${dateCondition} 
      AND (status = 'Delivered' OR status = 'Received')
    ...
    const result = {
      ...
      totalQuantity: Number(quantityResult[0].totalQuantity),
...
async function handleUpdateStocks(req, res, orderId) {
  const connection = await db.getConnection();
  
  try {
    await connection.beginTransaction();

    // First, get the ordered products for this order
    ...
    // Update the order status without touching order_date
    await connection.query(`
      UPDATE orders
      SET status = 'Return Cancelled'
      WHERE id = ?
    `, [orderId]);

    await connection.commit();
    res.status(200).json({ message: 'Stocks and order status updated successfully' });
  } catch (error) {
    await connection.rollback();
    console.error('Error updating stocks:', error);
    res.status(500).json({ error: 'An error occurred while updating stocks' });
  } finally {
    connection.release();
  }
}
